...
这里，我们使用更加方便的`InteractiveSession`类。通过它，你可以更加灵活地构建你的代码。它能让你在运行图的时候，插入一些构建[计算图](SOURCE/get_started/basic_usage.md#the-computation-graph)的操作。这能给使用交互式文本shell如iPython带来便利。如果你没有使用`InteractiveSession`的话，你需要在开始session和[加载图](SOURCE/get_started/basic_usage.md#launching-the-graph-in-a-session)之前，构建整个计算图。
...
所以，这部分Python代码，目的是构建这个在外部运行的计算图，并安排这个计算图的哪一部分应该被运行。详细请阅读[计算图](SOURCE/get_started/basic_usage.md#the-computation-graph)
部分的[基本用法](SOURCE/get_started/basic_usage.md)。
