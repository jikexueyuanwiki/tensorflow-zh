...
* 算法学习参数的[移动平均值](../../api_docs/python/train.md#ExponentialMovingAverage)的常用计算方式，以及在评估阶段使用这些平均值提高预测性能；  
...
本教程中的模型是一个多层架构，由卷积层和非线性层(nonlinearities)交替多次排列后构成。这些层最终通过全连通层对接到softmax分类器上。这一模型除了最上层的几层外，基本跟[Alex Krizhevsky](https://code.google.com/p/cuda-convnet/)提出的模型一致。    

在一个GPU上经过几个小时的训练后，该模型达到了最高86%的精度。细节请查看[下面](#evaluating-a-model)的描述以及代码。模型中包含了1,068,298个学习参数，分类一副图像需要大概19.5M个乘加操作。
...
`norm1` | [局部响应归一化](../../api_docs/python/nn.md#local_response_normalization).
`conv2` | [卷积](../../api_docs/python/nn.md#conv2d) and [rectified linear](../../api_docs/python/nn.md#relu) activation.
`norm2` | [局部响应归一化](../../api_docs/python/nn.md#local_response_normalization).
`pool2` | [max pooling](../../api_docs/python/nn.md#max_pool).
`local3` | [基于修正线性激活的全连接层](../../api_docs/python/nn.md).
`local4` | [基于修正线性激活的全连接层](../../api_docs/python/nn.md).
...
相比于总损失，在训练过程中的单项损失尤其值得人们的注意。但是由于训练中使用的数据批量比较小，损失值中夹杂了相当多的噪声。在实践过程中，我们也发现相比于原始值，损失值的移动平均值显得更为有意义。请参阅脚本[`ExponentialMovingAverage`](../../api_docs/python/train.md#ExponentialMovingAverage)了解如何实现。

## 评估模型 <a class="md-anchor" id="evaluating-a-model"></a>  

现在可以在另一部分数据集上来评估训练模型的性能。脚本文件`cifar10_eval.py`对模型进行了评估，利用 `inference()`函数重构模型，并使用了在评估数据集中的10,000张CIFAR-10图片进行测试。最终计算出的精度为*1:N*，N=预测值中置信度最高的一项与图片真实label匹配的次数。(It calculates the *precision at 1:* how often the top prediction matches the true label of the image)。
...
...
```
评估脚本只是周期性的返回precision@1 (The script merely returns the precision @ 1 periodically)--在该例中返回的精度是86%。`cifar10_eval.py` 同时也返回其它一些可以在TensorBoard中进行可视化的简要信息。可以通过这些简要信息在评估过程中进一步的了解模型。

训练脚本会为所有学习变量计算其[移动均值](../../api_docs/python/train.md#ExponentialMovingAverage),评估脚本则直接将所有学习到的模型参数替换成对应的移动均值。这一替代方式可以在评估过程中提升模型的性能。  
...
如果天真的对模型参数的采用异步方式更新将会导致次优的训练性能，这是因为我们可能会基于一个旧的模型参数的拷贝去训练一个模型。但与此相反采用完全同步更新的方式，其速度将会跟最慢的模型一样慢(Conversely, employing fully synchronous updates will be as slow as the slowest model replica.)。
...
可以看到，每一个GPU会为每一批次的数据计算梯度和inference。这种设置可以非常有效的将一大批数据分割到各个GPU上。

这一机制要求所有GPU能够共享模型参数。但是众所周知在GPU之间传输数据非常的慢，因此我们决定在CPU上存储和更新所有模型的参数(对应图中绿色矩形的位置)。这样一来，当所有GPU需要处理一批新的数据时只需要传递新的模型参数即可。
...
我们首先需要把在单个模型拷贝中计算inference和梯度的行为抽象到一个函数中。在代码中，我们称这个抽象对象为“tower”。对于每一个“tower”我们都需要设置它的两个属性：  
* 在一个tower中为所有操作设定一个唯一的名称。[`tf.name_scope()`](../../api_docs/python/framework.md#name_scope)通过添加一个范围前缀来提供该唯一名称。比如，第一个tower中的所有操作都会附带一个前缀`tower_0`，示例：`tower_0/conv1/Conv2D`；

* 在一个tower中运行操作的硬件设备类型。 [`tf.device()`](../../api_docs/python/framework.md#device) 提供该信息。比如，在第一个tower中的所有操作都位于 `device('/gpu:0')`范围中，暗含的意思是这些操作应该运行在第一块GPU上；
...
为了在多个GPU上共享变量，所有的变量都绑定在CPU上，并通过[`tf.get_variable()`](../../api_docs/python/state_ops.md#get_variable)访问。可以查看[Sharing Variables](../../how_tos/variable_scope/index.md)以了解如何共享变量。

### 启动并在多个GPU上训练模型 <a class="md-anchor" id="AUTOGENERATED-launching-and-training-the-model-on-multiple-gpu-cards"></a>

如果你的机器上安装有多块GPU，你可以通过使用`cifar10_multi_gpu_train.py`脚本来加速模型训练。该脚本是训练脚本的一个变种，用于在多个GPU之间实现模型并行训练。
