...
### 文件名, 乱序(shuffling), 和最大文件采样数(epoch limits) <a class="md-anchor" id="AUTOGENERATED-filenames--shuffling--and-epoch-limits"></a>
...
`string_input_producer` 提供的可配置参数来设置文件名乱序和最大的文件采样数， `QueueRunner`会将所有采样出来的文件名加入文件名队列中， 如果`shuffle=True`的话， 会对文件名进行乱序处理。这一采样过程是比较均匀的，因此它可以产生的均衡的文件名队列。
...
在数据输入管线的末端， 我们需要有另一个队列来执行输入样本的训练，评价和推理。因此我们使用[`tf.train.shuffle_batch` function](tensorflow-zh/SOURCE/api_docs/python/io_ops.md#shuffle_batch)来对队列中的样本进行乱序处理
...
如果你需要对不同文件中的样子有更强的乱序和并行处理，可以使用[`tf.train.shuffle_batch_join` function](tensorflow-zh/SOURCE/api_docs/python/io_ops.md#shuffle_batch_join).
示例:
